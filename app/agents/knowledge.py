...
class KnowledgeAgent:
    ...
    async def ingest(self, payload: dict) -> bool:
        ...
        text = raw_text.strip()
        # Store normalized non-whitespace text so deterministic fallbacks never
        # return an apparently empty quote.
        ...
        if text and len(text) >= self.settings.min_embed_chars:
            try:
                ...
                # Preserve raw memory even if free embedding quota/network is down.
                embedding = None
        return await self.db.upsert_message(payload, embedding)

    def _temporal_window(self, question: str) -> tuple[datetime, datetime] | None:
        q = question.lower()
        try:
    ...
    @staticmethod
    def _received_only(question: str) -> bool:
        q = question.lower()
    ...
    async def answer(self, tenant_id: str, question: str) -> AgentAnswer:
        temporal = self._temporal_window(question)
        received_only = self._received_only(question)
        ...
        # Never let whitespace / system-like records become answer evidence.
        hits = [h for h in hits if str(h.get('text') or '').strip()]
        if not hits:
            if temporal:
                return AgentAnswer(
                    agent='knowledge',
                    answer='I could not find indexed messages in that time range. If ThreadBoss was connected later, run `/sync 24h` (or `/sync 7d`) to backfill WAHA history.',
                    sources=[],
                )
        ...
        try:
            ...
            # Deterministic fallback that is still useful if free generation quota
            # is unavailable. Return multiple real evidence snippets, never blank.
            selected = hits[:5]
