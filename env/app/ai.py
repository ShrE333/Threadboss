...
class AIClient:
    ...
    ThreadBoss never silently enables a paid API. V1.6 also has a Gemini model
    ...
    async def _gemini_chat(
        ...
        candidates: list[str] = []
        for candidate in [model, self.settings.gemini_fallback_model, *[x.strip() for x in self.settings.gemini_model_fallbacks_csv.split(',') if x.strip()]]:
        ...
        for index, candidate in enumerate(candidates):
            ...
            try:
                ...
                if r.status_code in {404, 429, 503} and index + 1 < len(candidates):
                    logger.warning(
                        'Gemini generation model %s returned %s; retrying with %s',
                        candidate,
                        r.status_code,
                        candidates[index + 1],
                    )
