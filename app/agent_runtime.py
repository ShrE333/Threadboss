...
class AgentRuntime:
    ...
    async def ingest(self, payload: dict) -> bool:
        inserted = await self.knowledge.ingest(payload)
        # Planner only inspects a message once, after a successful new insert.
        if inserted and self.settings.enable_planner_extraction:
